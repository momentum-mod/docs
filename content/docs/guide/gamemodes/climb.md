...
** In the direction you are moving (not facing), 36 units down and 16 units forward from the player’s origin (This works out to be the bottom edge of your collision box) spawn a box of identical dimensions (32 unit width, 32 unit depth,72 unit height) to the player’s collision box down from that point, move the box down 34 units while checking if it touches anything. If there was nothing in its path, the player is next to an edge.  
 To simplify, if the player were to move an additional 16 units in their current direction and fall straight down 70 units would they, at any time, collide with anything?
