...
### .RES Generator (RESGen)
Download: https://gamebanana.com/tools/4777

RESGen is a tool used to create a resource file based on a provided map. The use case for this tool in the porting process is that it allows you to quickly find a list of all the assets a map uses. This is helpful for when you are trying to gather all the .wad files, models, and sounds your map uses. 
...
If you find yourself missing any additional textures they are likely Valve textures from Half-Life, Team Fortress Classic, or Counter-Strike that the author did not pack into their map. These are likely going to be from the large .wad files like halflife.wad that contain the bulk of these games' default textures. See the section "[Finding missing assets with RESGen](#finding-missing-assets-with-resgen)" under "[Other common issues](#other-common-issues)" later on in the guide for more help in finding missing .wad files. 
...
{{< hint info >}}Expect to encounter issues during this proccess. If you are having issue with VTFEdit while converting textures refer to the relevant sections under "[Other common issues](#other-common-issues)" at the bottom of this guide.{{< /hint >}}
...
Start by making sure the right flags are set for the trigger_push. If only the player needs to be moved, all you need is the "Clients" flag. Sometimes you will find trigger_push entities that are designed to affect other objects like conc grenades for instance. In this case you should select the "Everything" flag.
...
The output from running Xwad will be in your materialsrc folder for the game you are using. Using Team Fortress 2 as an example, the output path is `steamapps\common\team fortress 2\tf\materialsrc`

Now you will need to open up VTFEdit and go to File -> Import. In the window that opens you will need to select all of the .tga files generated by Xwad in the path listed above. After selecting all of these images a settings panel will open. If you know what you are doing tinker with these settings to your liking. Otherwise under the "General" tab, uncheck the box "Generate Mipmaps." Next under the "Resources" tab, uncheck the box "Create LOD Control Resource." Once you are done modifying the settings click "OK." You will now have a .vtf file for your sprite. Save it to your `materials/sprites` folder of the game you are working with. You will now need to create a .vmt file for your sprite. Below is an example .vmt file you can copy and paste. Be sure to change the value of $basetexture to the actual name of your sprite's .vtf file.
...
"Sprite"
{
	...
	"$basetexture" "sprites/MyEpicSpriteName"
}
```

Be warned, some sprite files may not convert perfectly and will require manually editing the texture in image-editing software. Also, here is a reminder that by default Hammer hides sprite files when browsing for textures. Either edit the .fgd file to make them show up, or manually enter the name of the file into the relevant key value of the env_sprite. In other words manually fill in the "Sprite Name" key value with `sprites/my_sprite.vmt`
...
### Finding missing assets with RESGen
A problem you may face in the porting process is having a .bsp file that looks fine when loaded in its original game, but you cannot seem to find its missing textures/models/sounds for porting. This is where you will want to use the tool RESGen linked in the "Optional software" section of the guide. Download the tool and extract the contents somewhere on your computer. All you need to do is take the .bsp file of the map you are working on, and then drag it on to RESGen.exe. The tool will output a file called mymapname.res. Open this file in a text editor. Here you will see a list of all the assets your map uses. If you search your Half-Life/Team Fortress Classic/Counter-Strike folder for these assets you should be able to find them. An example output is provided below.
```
// quadrocket_adam2.res - created with RESGen v2.0.2.
...
models/pent.mdl
models/quad.mdl
sound/adam2/adam2-1.wav
sound/adam2/temple.wav
...
tfc2.wad
```
Everything here execpt the sound files adam2-1.wav and temple.wav are default Valve assets.
